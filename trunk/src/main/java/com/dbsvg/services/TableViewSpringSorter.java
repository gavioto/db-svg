 ...
 * 
 * Explanation & pseudocode taken from Wikipedia: Pseudocode
 * 
...
public class TableViewSpringSorter implements Serializable, ITablePageSorter {
	...
	protected static final Logger LOG = LoggerFactory
			.getLogger(TableViewSpringSorter.class);
	...
	public List<TableView> SortPage(SchemaPage currentPage, boolean resort) {
		// int i = 0;
		relativeAttraction = new double[currentPage.getTableViews().size()][currentPage
				.getTableViews().size()];
		List<TableView> tableViewswDirtyValues = new ArrayList<TableView>();
		...
		if (tableViewswDirtyValues.size() > 0 || resort) {
			...
			List<TableView> tableViewstoSort = new ArrayList<TableView>();
			if (resort) {
			...
			// We only need to go through half of the tables on the inner
			// iteration, so we shortcut it.
			List<TableView> TVToGoThrough = new ArrayList<TableView>();
			...
			// for (TableView tv : tableViews) {
			// LOG.debug("Table:"+tv.getTable().getName() +
			// " "+tv.getX()+","+tv.getY());
			// }
			...
			int n = 0;
			long total_kinetic_energy = 0; // running sum of total kinetic
											// energy over all particles
			do {
				n++;
				LOG.debug("Spring Iteration: " + n);
				total_kinetic_energy = 0; // reset to 0 each round.
				for (TableView tv1 : tableViewstoSort) {
					long[] net_force = { 0, 0 }; // running sum of total force
					...
					for (TableView tv2 : tableViews) {
						if (tv1 != tv2) {
							LOG.debug("Dist to " + tv2.getTable().getName()
									+ ": " + tv1.calcDistanceWRadius(tv2)
									+ " θ:" + tv1.calcAngle(tv2));
							LOG.debug("bf: " + calcBoundaryForce(tv1, tv2));
							long energy = 0;
							// energy += calcForce(tv1,tv2);//net-force :=
							// net-force + Coulomb_repulsion( this_node,
							// other_node )
							energy += calcHookeForce(tv1, tv2);// net-force :=
																// net-force +
																// Hooke_attraction(
																// this_node,
																// spring )
					...
					net_force[0] += (hitBoundary ? 0.4 : 1)
							* calcGravity(tv1.getX() - CENTER_X);
					net_force[1] += (hitBoundary ? 0.4 : 1)
							* calcGravity(tv1.getY() - CENTER_Y);
					LOG.debug("G:" + net_force[1]);

					// without damping, it moves forever
					// this_node.velocity := (this_node.velocity + timestep *
					// net-force) * damping
					tv1.setVelocityX((tv1.getVelocityX() + TIME_STEP
							* net_force[0])
							* DAMPING);
					tv1.setVelocityY((tv1.getVelocityY() + TIME_STEP
							* net_force[1])
							* DAMPING);
					...
					// total_kinetic_energy += NODE_MASS *
					// (this_node.velocity)^2;
					total_kinetic_energy += NODE_MASS
							* Math.pow(tv1.getVelocityX(), 2)
							+ Math.pow(tv1.getVelocityY(), 2);

					LOG.debug("Table:" + tv1.getTable().getName() + " "
							+ tv1.getX() + ", " + tv1.getY() + " Vx:"
							+ tv1.getVelocityX() + " Vy:" + tv1.getVelocityY());
				}

				LOG.debug("  KE: " + total_kinetic_energy);
			} while ((n < 10)
					|| (total_kinetic_energy > 1000 && n < tableViews.size() * 50));
	...
	/**
	 * Coulomb and Hooke's Law's weren't working right in this model, so I made
	 * my own force equation
	 * 
	 * -1 * relative Attraction * distance
	 * 
	 * 
	 * A positive force implies a repulsive interaction, while a negative force
	 * implies an attractive interaction.
	 * 
	 * @return
	 */
	protected double calcForce(TableView tv1, TableView tv2) {
		...
		LOG.debug(relativeAttraction[tv1.getId()][tv2.getId()] + " Force of "
				+ tv2.getTable().getName() + " " + tv1.calcDistanceWRadius(tv2)
				+ " away on " + tv1.getTable().getName() + ": " + force);
		return force;
	}
	 ...
	 * my own force equation
	 * 
	 * for negative values (means there is a collision) -3 * relative Attraction
	 * * distance^2 for positive values relative Attraction * distance^2
	 * 
	 * 
	 * A positive force implies a repulsive interaction, while a negative force
	 * implies an attractive interaction.
	 ...
	 * 
	 * gravity = A positive force implies a repulsive interaction, while a
	 * negative force implies an attractive interaction.
	 * 
	 * @return
	 */
	 ...
	 * @return
	 */
	protected double calcCoulombForce(TableView tv1, TableView tv2) {
		double ccf = COULOMBS_CONSTANT
				* TIME_STEP
				* ((relativeAttraction[tv1.getId()][tv2.getId()]) / (tv1
						.calcDistanceWRadius(tv2) * 1000));
		LOG.debug("CC: " + ccf);
		return ccf;
	}
	 ...
	 * 
	 * The distance x includes the radius of the table, so overlapping tables
	 * will repel each other.
	 * 
	 * the force constant should never be zero or negative, so we replace those
	 * values with .1
	 * 
	 * A positive force implies a repulsive interaction, while a negative force
	 * implies an attractive interaction.
	...
	private double calcHookeForce(TableView tv1, TableView tv2) {
		double attraction = relativeAttraction[tv1.getId()][tv2.getId()];
		LOG.debug("HKa:" + attraction);
		double hookeForce = -.1 * attraction
				* (tv1.calcDistanceWRadius(tv2) - OPTIMAL_DISTANCE);
		LOG.debug("HKe:" + hookeForce);
		return hookeForce;
	}
	...
	/**
	 * Calculates the relatative attractive force of two tables. If we are
	 * comparing the table with itself, the attraction is 0. The further away a
	 * node is linked, the lesser the attraction. if no link is found within 4
	 * nodes, the attraction is negative.
	 * 
	 * @param tv1
	 * @param tv2
	 * @return
	 ...
	 * of the first table. Uses some short-cutting to cut down on unnessary
	 * checking
	 * 
	 * @param level
